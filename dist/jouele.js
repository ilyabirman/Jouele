(function($) {
    ...
    var Helpers = {
        ...
        "formatTime": function(rawSeconds) {
        ...
        "getFirstJouele": function() {
            var first_jouele;
            var $first_jouele = $(".jouele").filter(".jouele_first").eq(0);

            if ($first_jouele.length === 0) {
                if ($.Jouele.playlist[0][0] instanceof Jouele) {
                    first_jouele = $.Jouele.playlist[0][0];
                }
            } else {
    ...
    var Handlers = {
        ...
        "timeline": function(event) {
            ...
            var JoueleInstance = $timeline_seeking.parents(".jouele").eq(0).data("jouele");
            ...
            switch (event.type) {
                ...
                case "touchcancel":
                    ...
                    JoueleInstance.getTrack().player["seekingOnTimeline"]["seekingElement"] = false;
    ...
    var Redraw = {
        ...
        "updateTitle": function() {
            ...
            if (title && title !== JoueleInstance.getTrack().player["title"]) {
        ...
        "updateTimeDisplay": function() {
            ...
            function calculateTime() {
                if (JoueleInstance.getTotalTime() || JoueleInstance.getTrack().player["isBroken"]) {
                    total_time = JoueleInstance.getTotalTime();
                ...
                remaining_time = total_time ? Math.round(Helpers.makeSeconds(total_time) - Helpers.makeSeconds(elapsed_time)) : 0;
                elapsed_time = JoueleInstance.getTrack().player["isBroken"] ? 0 : Math.round(elapsed_time);
            ...
            function showTime() {
                for (var i = 0; i < JoueleInstance.getTrack().instances.length; i++) {
                    if (JoueleInstance.getTrack().player["isBroken"]) {
                        ...
                        JoueleInstance.getTrack().instances[i].$container.find(".jouele-info-time__total").text(total_time ? Helpers.formatTime(total_time) : "");
                        JoueleInstance.getTrack().instances[i].$container.find(".jouele-info-time__current").text(total_time ? Helpers.formatTime(elapsed_time) : "");
                    }
                }
            }
            ...
            if (JoueleInstance.$container.length > 0 && (total_time || JoueleInstance.getTrack().player["isStarted"] || JoueleInstance.getTrack().player["seekTime"] || JoueleInstance.getTrack().player["seekingOnTimeline"]["isSeeking"])) {
                showTime();
            }
    ...
    var Init = {
        ...
        "getPlaylistIndex": function() {
            ...
            $(".jouele-playlist").add(".jouele:not(.jouele-playlist .jouele)").each(function(index, element) {
            ...
            if (JoueleInstance.getPlaylistDOM().length > 0) {
                var $jouele_links = JoueleInstance.getPlaylistDOM().find(".jouele").filter("[href]");
        ...
        "initInnerControls": function() {
            ...
            JoueleInstance.$container.find(".jouele-progress-line").off("mousedown.jouele touchstart.jouele").on("mousedown.jouele touchstart.jouele", function(event) {
                ...
                JoueleInstance.getTrack().player["seekingOnTimeline"]["seekingElement"] = $timeline;
            ...
            var createInfoAreaDOM = function() {
                return [
                    $(document.createElement("div")).addClass("jouele-info-time").append(
                        ...
                        $(document.createElement("div")).addClass("jouele-info-time__total").text(JoueleInstance.getTotalTime() ? Helpers.formatTime(Helpers.makeSeconds(JoueleInstance.getTotalTime())) : "")
                    ),
    ...
    var Core = {
        "createHowler": function() {
            ...
            if (typeof Howl === "undefined") {
                Helpers.showError("Please include howler.js into your page — it is necessary for Jouele");
                return false;
            }
        ...
        "play": function() {
            ...
            var nearest_second = ((Math.ceil(JoueleInstance.getTrack().player["howler"].seek()) - JoueleInstance.getTrack().player["howler"].seek()) * 1e3) / 1e3;
            setTimeout(function() {
                Redraw.updateState.call(JoueleInstance);
        ...
        "pause": function() {
            ...
            if (JoueleInstance.getTrack().player["howler"] && JoueleInstance.getTrack().player["isLoaded"]) {
        ...
        "playFrom": function(time) {
            ...
            if (JoueleInstance.getTrack().player["isLoaded"]) {
                ...
                setTimeout(function() {
                    if (current_time === JoueleInstance.getTrack().player["howler"].seek()) {
                        Preloader.show.call(JoueleInstance);
                    }
                    interval = setInterval(function() {
                        if (current_time !== JoueleInstance.getTrack().player["howler"].seek()) { // Seeking part is loaded and playing
                            Preloader.hide.call(JoueleInstance);
        ...
        "destroy": function() {
            ...
            if (JoueleInstance.getTrack().player["seekingOnTimeline"]["seekingElement"][0] === JoueleInstance.$container.find(".jouele-progress-line")[0]) {
                $(document).off("mouseup.jouele touchend.jouele touchcancel.jouele mousemove.jouele touchmove.jouele");
                JoueleInstance.getTrack().player["seekingOnTimeline"]["isSeeking"] = false;
                JoueleInstance.getTrack().player["seekingOnTimeline"]["seekingElement"] = false;
                JoueleInstance.getTrack().player["seekingOnTimeline"]["seekingPosition"] = null;
            }
            ...
            function removeTrack() {
                ...
                delete tracks[JoueleInstance.getHref()];
            }
            ...
            JoueleInstance.$link.removeData("jouele");
        ...
        Events: {
            ...
            "onSeek": function() {
                ...
                if (JoueleInstance.getTrack().player["seekTime"] !== null && (JoueleInstance.getTrack().player["howler"].seek() !== JoueleInstance.getTrack().player["seekTime"]) && (JoueleInstance.getTrack().player["seekTime"] !== JoueleInstance.getTotalTime()) && (JoueleInstance.getTrack().player["seekPosition"] !== 100)) {
                ...
                if (JoueleInstance.getTrack().player["seekTime"] === JoueleInstance.getTotalTime()) {
                    /* Hacks for Howler 2.0.5 */
                    if (JoueleInstance.isPlayed() && JoueleInstance.isPlaying()) {
                        JoueleInstance.pause(); // This is mandatory because Howler will check `paused` state in `_ended` event
                    } else {
    ...
    var Jouele = function($element, options) {
        ...
        function checkNewOptions(old_options, new_options) {
        ...
        this.getTotalTime = function() { return typeof this.getTrack() !== "undefined" ? this.getTrack().player["totalTime"] : false; };
        this.getElapsedTime = function() { return typeof this.getTrack() !== "undefined" ? this.getTrack().player["elapsedTime"] : false; };
        this.getRemainingTime = function() { return typeof this.getTrack() !== "undefined" ? this.getTrack().player["remainingTime"] : false; };
        ...
        this.setOptions = function(new_options) {
            options = $.extend(
                options,
                checkNewOptions(options, new_options)
            );
        ...
        /* Set API methods */
        this.play = function() {
            return typeof this.getTrack() !== "undefined" ? this.getTrack().player.play.call(this) : false;
        };
        this.pause = function() {
            return typeof this.getTrack() !== "undefined" ? this.getTrack().player.pause.call(this) : false;
        };
        this.playFrom = function(time) {
            return typeof this.getTrack() !== "undefined" ? this.getTrack().player.playFrom.call(this, time) : false;
        };
        this.seek = function(seekPositionPercent) {
            return typeof this.getTrack() !== "undefined" ? this.getTrack().player.seek.call(this, seekPositionPercent) : false;
        };
        this.destroy = function() {
            return typeof this.getTrack() !== "undefined" ? Core.destroy.call(this) : false;
        };
