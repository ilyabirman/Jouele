...
Для быстрой перемотки длинных файлов сервер должен поддерживать HTTP-запросы диапазонов (`Range`) и отвечать статусом `206 Partial Content`. Жуэль применяет выбранную позицию сразу после получения метаданных аудио, а браузер сам запрашивает соответствующую часть файла. Это работает и для файлов с переменным битрейтом.
...
Тип: `Boolean`
По умолчанию: `false`
При значении `data-static="true"` ссылка отображается как плеер в начальном состоянии, но остаётся полностью статичной: аудио не загружается, элементы управления не реагируют на действия пользователя, а плеер не попадает в треки, плейлисты и историю Жуэля. Продолжительность можно указать с помощью `data-length`. Исходный `href` ссылки сохраняется.
```html
<a href="song.mp3" class="jouele" data-static="true" data-length="2:30">Song</a>
```
Только точное строковое значение `"true"` включает статичный режим.
...
Изменяет опции уже инициализированного плеера или контрола без его уничтожения и повторной инициализации. Например, позволяет обновить название и продолжительность, включить повтор, поменять скин или отображение таймлайна и изменить управление пробелом. Опции `href` и `static` после инициализации не меняются.
